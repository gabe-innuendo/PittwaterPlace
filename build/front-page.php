<?php require_once('header.php'); ?>
    <main class="front-page-slider-area">
        <ul>
            <li class="slide-container" style="background-image: url(images/temp_images/slider/slide-1.jpg);">
                <div class="slide-text-container">
                    <div class="slide-text">
                        <h2>A commitment to fresh produce</h2>
                        <p>Pittwater Place is the central district in Mona Vale for buying fresh produce and ingredients.</p>
                        <a href="#" class="link-button">See our retailers</a>
                    </div>
                </div>
            </li>
            <li class="slide-container" style="background-image: url(images/temp_images/slider/slide-2.jpg);">
                <div class="slide-text-container">
                    <div class="slide-text">
                        <h2>Seasonal Cooking Demonstrations</h2>
                        <p>Some expert chefs will be at Pittwater place this weekend October 20 giving cooking demonstrations.</p>
                        <a href="#" class="link-button">Book your place</a>
                    </div>
                </div>
            </li>
        </ul>
    </main>
    <section class="front-page-main-boxes">
        <div class="flexGridRow flexGridRow--wrap">
            <div class="cell cell--oneThird">
                <a href="">
                    <div class="main-box" style="background-image: url(images/temp_images/main_boxes/fruit.jpg)">
                        <div class="main-box-text-container">
                            <h2>Store Finder</h2>
                        </div>
                    </div>
                </a>
            </div>
            <div class="cell cell--oneThird">
                <a href="">
                    <div class="main-box" style="background-image: url(images/temp_images/main_boxes/coffe.jpg)">
                        <div class="main-box-text-container">
                            <h2>What's Happening</h2>
                        </div>
                    </div>
                </a>
            </div>
            <div class="cell cell--oneThird">
                <a href="">
                    <div class="main-box" style="background-image: url(images/temp_images/main_boxes/whats-on.jpg)">
                        <div class="main-box-text-container">
                            <h2>Open today<br />
                            <span class="thin-text">9:30-5:30pm</span></h2>
                        </div>
                    </div>
                </a>
            </div>
        </div>
    </section>
<?php require_once('footer.php'); ?>
